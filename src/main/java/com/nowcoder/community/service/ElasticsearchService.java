...
public class ElasticsearchService {
    ...
    public SearchResult searchDiscussPost(String keyword, int current, int limit) throws IOException {
        SearchRequest request = new SearchRequest("discusspost"); //discuss是索引名，对应数据库的表名
        // 高亮显示
        ...
        for (SearchHit hit : response.getHits().getHits()) {
            ...
            if (titleField != null) {
                discussPost.setTitle(titleField.fragments()[0].toString());
            }
            HighlightField contentField = hit.getHighlightFields().get("content");
            if (contentField != null) {
                discussPost.setTitle(contentField.fragments()[0].toString());
            }
